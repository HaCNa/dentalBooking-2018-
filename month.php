<?php
  ...
  $query = "SELECT YEAR(booking_date) as SalesYear, MONTH(booking_date) as SalesMonth, SUM(service_price) AS TotalSales
			FROM booking GROUP BY YEAR(booking_date), MONTH(booking_date) ORDER BY YEAR(booking_date), MONTH(booking_date)";
	...
	axisX: {
		title: "Month",
			...
			data: [
			{
				...
				{ y:<?php echo $row['TotalSales'] ?>,label: "<?php echo $row['SalesMonth'] ?> / <?php echo $row['SalesYear'] ?>"},
...
<div class="content">
	...
	<li><a class="active" href="month.php">Sales per Month</a></li>
	</ul>
